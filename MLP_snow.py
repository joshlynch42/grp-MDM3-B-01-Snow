...
def plot_nn(X_test, predictions, y_test):
    X = np.linspace(0, 1, len(X_test))
    plt.figure(figsize=(8, 5))
    plt.plot(X, predictions, 'r-', label='MLP')
...
def MLP_fit(column_name, plot):
    ...
    X_train, X_test, y_train, y_test = test_train_split(df, column_name)
    X_train, X_test = scaling_values(X_train, X_test)

    MLP = MLPClassifier(hidden_layer_sizes=(20, 8, 6, 5), max_iter=2000)  # Creating an MLP neural network
    MLP.fit(X_train, y_train)  # Fitting neural network using train values

    predictions = MLP.predict(X_test)  # Predicting values for the test data

    rms = mean_squared_error(y_test, predictions, squared=False)  # Calculating root mean squared error
    if plot == 'yes':
        plot_nn(X_test, predictions, y_test)
    return rms


# Using a for loop to calculate average rms
total_rms = 0  # Initialise a total counter
num_it = 2  # Number of iterations
for i in range(num_it):
    print('Iteration number ' + str(i))
    if i < num_it - 1:
        total_rms += MLP_fit(column_name, 'no')  # Adding rms to mean total
    else:
        total_rms += MLP_fit(column_name, 'yes')


print('average rms is ' + str(total_rms/num_it))  # Calculating and printing mean rms
